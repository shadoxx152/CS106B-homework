...
Vector<DataPoint> mergeSort(const Vector<DataPoint>& subsequences_left, const Vector<DataPoint>& subsequences_right) {
    Vector<DataPoint> mergedVector;

    size_t leftIndex = 0, rightIndex = 0;
    while (leftIndex < subsequences_left.size() && rightIndex < subsequences_right.size()) {
        DataPoint left = subsequences_left[leftIndex];
        DataPoint right = subsequences_right[rightIndex];
        if (left.weight < right.weight) {
            mergedVector.add(left);
            leftIndex++;
            continue;
        }

        mergedVector.add(right);
        rightIndex++;
    }

    while (leftIndex < subsequences_left.size()) {
        mergedVector.add(subsequences_left[leftIndex++]);
    }

    while (rightIndex < subsequences_right.size()) {
        mergedVector.add(subsequences_right[rightIndex++]);
    }
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> leftRes = combine(left);
    Vector<DataPoint> rightRes = combine(right);

    return mergeSort(leftRes, rightRes);
}
