...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for (size_t i = 0;i + kGramLength <= str.length();++i) {
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key : input.keys()) {
        frequencies[key] = input[key] / sqrt(squareSum);
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    Map<string, double> topKGramsMap;

    for (string gram : source.keys()) {
        if (pq.size() < numToKeep) {
            pq.enqueue(gram, source[gram]);
        }
        else {
            if (source[gram] > pq.peekPriority()) {
                pq.dequeue();
                pq.enqueue(gram, source[gram]);
            }
        }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (Corpus corpus : corpora) {
        double sim = cosineSimilarityOf(textProfile, corpus.profile);
        if (sim > max) {
            max = sim;
            name = corpus.name;
        }
