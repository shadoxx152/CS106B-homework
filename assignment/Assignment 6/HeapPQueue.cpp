...
void HeapPQueue::enqueue(const DataPoint& data) {
    if (logicalSize == allocatedSize - 1) {
        allocatedSize *= 2;
        DataPoint* newSpace = new DataPoint[allocatedSize];
        for (size_t i = 1;i <= logicalSize;++i) {
            newSpace[i] = elems[i];
        }
        delete[] elems;
        elems = newSpace;
    ...
    while (parent > 0 && elems[cur].weight < elems[parent].weight) {
        swap(elems[cur], elems[parent]);
        cur = parent;
        parent = cur / 2;
    }
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    while (pos <= logicalSize / 2 && pos < logicalSize) {
        size_t cur = 2 * pos;
        size_t right = cur + 1;
        if (right <= logicalSize && elems[right].weight < elems[cur].weight) {
            cur = right;
        }

        if (elems[pos].weight < elems[cur].weight) {
            break;
        }
        swap(elems[pos], elems[cur]);
        pos = cur;
    }
