...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    string name = possibleLinks.firstKey();
    Set<string> possibleLink = possibleLinks[name];
    Map<string, Set<string>> newPossibleLinks = possibleLinks;

    newPossibleLinks.remove(name);

    for (const string& partner : possibleLink) {
        if (!possibleLinks.containsKey(partner)) {
            continue;
        }

        Set<string> partnerSet = possibleLinks[partner];

        if (partnerSet.contains(name)) {
            Pair newMatching = {name, partner};

            newPossibleLinks.remove(partner);

            matching.add(newMatching);
            if (hasPerfectMatching(newPossibleLinks, matching)) {
                return true;
            }
            matching.remove(newMatching);
            newPossibleLinks[partner] = partnerSet;
        }
    }
    return false;
}

void backtrack(Map<string, Map<string, int>> remainingLinks, Set<Pair>& currentMatching,
               int currentWeight, Set<Pair>& bestMatching, int& maxWeight) {
    ...
    string name = remainingLinks.firstKey();
    Map<string, int> links = remainingLinks[name];
    remainingLinks.remove(name);

    for (const string& partner : links) {
        if (!remainingLinks.containsKey(partner)) continue;

        int weight = links[partner];

        Map<string, Map<string, int>> newRemainingLinks = remainingLinks;
        newRemainingLinks.remove(partner);

        currentMatching.add({name, partner});
        backtrack(newRemainingLinks, currentMatching, currentWeight + weight, bestMatching, maxWeight);
        currentMatching.remove({name, partner});
    }

    backtrack(remainingLinks, currentMatching, currentWeight, bestMatching, maxWeight);
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    backtrack(possibleLinks, currentMatching, 0, bestMatching, maxWeight);
