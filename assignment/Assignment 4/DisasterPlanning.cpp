 ...
 */
int countCoverage(const string& city, const Map<string, Set<string>>& roadNetwork,
                  const Set<string>& covered) {
    int count = covered.contains(city) ? 0 : 1;
...
bool backtrack(const Map<string, Set<string>>& roadNetwork, Vector<string>& cities,
               Set<string>& supplyLocations, Set<string>& covered,
               int index, int remainingSupplies) {
    if (remainingSupplies < 0) return false;
    if (covered.size() == roadNetwork.size()) return true;

    int uncovered = roadNetwork.size() - covered.size();
    const int maxCoverPerSupply = 5;
    int minSuppliesNeeded = (uncovered + maxCoverPerSupply - 1) / maxCoverPerSupply;
    if (remainingSupplies < minSuppliesNeeded) {
        return false;
    }

    if (index >= cities.size()) return false;
    ...
    if (backtrack(roadNetwork, cities, supplyLocations, covered, index + 1, remainingSupplies - 1)) {
        return true;
    }
    ...
    return backtrack(roadNetwork, cities, supplyLocations, covered, index + 1, remainingSupplies);
}
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    sort(cities.begin(), cities.end(), [&](const string& a, const string& b) {
        return countCoverage(a, roadNetwork, {}) > countCoverage(b, roadNetwork, {});
    });

    Set<string> covered;
    return backtrack(roadNetwork, cities, supplyLocations, covered, 0, numCities);
}
